...
class GrayImgTrainPreprocessor(_TrainTimePreprocessor):
    ...
    def run(self, images_train, labels_train, images_val, labels_val, nb_classes=2):
        ...
        X_train = np.array([self._to_gray(patch) for patch in images_train],
                           dtype='float').reshape(-1, n_rows, n_cols, 1)
        X_val = np.array([self._to_gray(patch) for patch in images_val],
                         dtype='float').reshape(-1, n_rows, n_cols, 1)
